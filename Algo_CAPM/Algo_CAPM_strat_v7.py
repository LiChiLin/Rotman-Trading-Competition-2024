...
def calculate_spread_z_score(alpha_mid_price, gamma_mid_price, historical_spreads):
    current_spread = alpha_mid_price - gamma_mid_price
...
def buy_or_sell(
    ...
    z_score = calculate_spread_z_score(
        alpha_mid_price, gamma_mid_price, historical_alpha_gamma_spreads
    )
...
def main():
    with requests.Session() as session:
        ...
        while get_tick(session) < 600 and not shutdown:
            ...
            alpha_mid_price = (
                (alpha["BID"].iloc[0] + alpha["ASK"].iloc[0]) / 2
                if not alpha.empty
                else np.nan
            )
            gamma_mid_price = (
                (gamma["BID"].iloc[0] + gamma["ASK"].iloc[0]) / 2
                if not gamma.empty
                else np.nan
            )
            print(alpha_mid_price)
            print(gamma_mid_price)
            # Calculate the spread and store it in the historical spreads list
            if not np.isnan(alpha_mid_price) and not np.isnan(gamma_mid_price):
                spread = alpha_mid_price - gamma_mid_price
