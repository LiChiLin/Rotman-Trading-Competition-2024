  ...
  for x in tickers:
    ...
    if diff<0 and (abs(pos)>=5000 or abs(diff)>0.35):
      ...
      if pos<0:
        ...
        if accept_sell[x]>0:
          accept_sell[x]-=0.5
        order_list[x]['BUY']+=[int(ritc.limit_buy(x,mmnum,bid+mmjump*5)['order_id'])]
      elif pos>0:
        ...
        if accept_buy[x]>0:
          accept_buy[x]-=0.5
        order_list[x]['SELL']+=[int(ritc.limit_sell(x,mmnum,ask-mmjump*5)['order_id'])]
    else:
      if accept_buy[x]<1:
        count_buy+=1
        if count_buy%5==0:
    ...
    if abs(ask-bid)>limit_a:
      if accept_buy[x]>0:
        order_list[x]['BUY']+=[int(ritc.limit_buy(x,mmnum*accept_buy[x],bid+mmjump)['order_id'])]
      if accept_sell[x]>0:
        order_list[x]['SELL']+=[int(ritc.limit_sell(x,mmnum*accept_sell[x],ask-mmjump)['order_id'])] 
    if x=='RITC':
