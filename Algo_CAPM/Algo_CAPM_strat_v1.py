...
def adjust_positions_for_limits(
    position_size_alpha, position_size_gamma, current_positions, gross_limit, net_limit
):
    ...
    # Calculate the potential gross and net exposure
    ...
    adjustment_needed = (
        potential_gross_position > gross_limit or potential_net_position > net_limit
    )
    ...
    if adjustment_needed:
        ...
        scale_factor_gross = (
            min(1, gross_limit / potential_gross_position)
            if potential_gross_position > gross_limit
            else 1
        )
        scale_factor_net = (
            min(1, net_limit / potential_net_position)
            if potential_net_position > net_limit
            else 1
        )


        # Apply the most restrictive scale factor to adjust positions
        scale_factor = min(scale_factor_gross, scale_factor_net)
        adjusted_position_size_alpha = (
            desired_alpha_position - current_alpha_position
        ) * scale_factor
        adjusted_position_size_gamma = (
            desired_gamma_position - current_gamma_position
        ) * scale_factor
    else:
...
def buy_or_sell(
    ...
    position_size_alpha = (
        base_position_size * (1 + expected_return_alpha)
        if expected_return_alpha > 0
        else base_position_size / 2
    )
    position_size_gamma = (
        base_position_size * (1 + expected_return_gamma)
        if expected_return_gamma > 0
        else base_position_size / 2
    )
    ...
    # Calculate gross and net positions after potential trades
    potential_gross_position = abs(
        current_positions["ALPHA"] + position_size_alpha
    ) + abs(current_positions["GAMMA"] + position_size_gamma)
    potential_net_position = abs(
        current_positions["ALPHA"]
        + position_size_alpha
        - (current_positions["GAMMA"] + position_size_gamma)
    )
    ...
    if z_score > entry_threshold:
        ...
        session.post(
            ...
            params={
                "ticker": "ALPHA",
                "type": "MARKET",
                "quantity": adjusted_position_size_alpha,
                "action": "SELL",
            },
        )
        ...
        session.post(
            ...
            params={
                "ticker": "GAMMA",
                "type": "MARKET",
                "quantity": adjusted_position_size_gamma,
                "action": "BUY",
            },
        )
    ...
    with requests.Session() as session:
        ...
        while get_tick(session) < 600 and not shutdown:
            ...
            if not np.isnan(alpha_last_price) and not np.isnan(gamma_last_price):
                ...
                if len(historical_alpha_gamma_spreads) > 30:
